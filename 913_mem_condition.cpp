...
int main(){
	...
	Total_number = 0; // 
        ...
        while(Total_number< 100000000){
            ...
            //*************************************
            for(int i=0; i<2; i++){
                for(int j=0; j<n; j++)	Error_vector[i][j] = 0; 
            }
    ...
    //====================================================================
			...
			if ( m1==1 || m2==1 || m3==1 || m4==1 || m5==1 || m6==1 ){
				...
				for(int i=0; i<2; i++){
                	for(int j=9; j<n; j++)	Error_vector[i][j] = 0; 
            	}
...
//---Perfect circuit----------------------------------------------------------------------------------------------------
		...
		// Exclusion stabilzer & degenerate error	
...
int SQerror(int (*Error_vector)[n], int a, double probability){
    a = a - 1;
    r = next64()/(pow(2,64));
    if( r < (probability/3) ){ 							// depolarizing channel, r < (1/3)*p, X error
        *(*(Error_vector+0)+a) = (*(*(Error_vector+0)+a) + 1)%2;
    }
    if( r > (probability/3) && r < (2*probability/3) ){ // (1/3)*p < r < (2/3)*p, Z error
        *(*(Error_vector+1)+a) = (*(*(Error_vector+1)+a) + 1)%2;
    }
    if( r > (2*probability/3) && r < (probability) ){ 	// (2/3)*p < r < p, Y error
        *(*(Error_vector+0)+a) = (*(*(Error_vector+0)+a) + 1)%2;
        *(*(Error_vector+1)+a) = (*(*(Error_vector+1)+a) + 1)%2;
    }
...
int CNOT(int (*Error_vector)[n], int a, int b){
    a = a - 1;
    b = b - 1;
    if(Error_vector[0][a]==1){                                     // X error 
        Error_vector[0][b] = (Error_vector[0][b] + 1)%2;
    }
    if(Error_vector[1][b]==1){    								   // Z error 
        Error_vector[1][a] = (Error_vector[1][a] + 1)%2;
    }
